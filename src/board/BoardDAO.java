...
public class BoardDAO {//DB작업 하는 클래스 
	...
	public void insertBoard(BoardBean bBean){ //<--입력한 새글정보는 매개변수로 전달 받음
		...
		try {
			...
			pstmt.setInt(6, num);//주글(새글)과 답변글을 묶어서 나타내는 그룹번호 
								// 여기서는 주글(새로추가할글)의 그룹번호
			pstmt.setInt(7, 0);//주글(새로추가할글)의 들여쓰기 정도 레벨값 0 
			pstmt.setInt(8, 0);//주글 순서
			pstmt.setInt(9, 0);//주글(새로추가할글)의 조회수 0
			pstmt.setTimestamp(10, bBean.getDate()); //주글을 추가한 날짜및 시간 
			pstmt.setString(11, bBean.getIp());//주글(새글)을 추가한 사람의  IP주소 
...
/*수정할 글정보(BoardBean)객체를 메소드의 매개변수로 전달받아.. DB에 저장된 비밀번호와 일치하면 글 수정*/
	...
	public void reInsertBoard(BoardBean bBean){
		...
		try{
			...
			pstmt.setString(4, bBean.getSubject());
			pstmt.setString(5, bBean.getContent());
			pstmt.setInt(6, bBean.getRe_ref()); //답변글 INSERT시 부모글의 그룹값을 사용하여 INSERT
												//이유 : 부모글과 답변글은 같은 그룹값을 가지기 위함
			pstmt.setInt(7,bBean.getRe_lev()+1);//부모글의 re_lev에  + 1을 사용하여 답변글 INSERT함
			pstmt.setInt(8,bBean.getRe_seq()+1);//부모글의 re_seq에  + 1을 사용하여 답변글 INSERT함
			pstmt.setInt(9, 0);//답변글 INSERT시 글조회수 0
		    pstmt.setTimestamp(10, bBean.getDate()); //답글을 작성한 날짜 	
			pstmt.setString(11, bBean.getIp());//답글 작성자의 IP주소 
