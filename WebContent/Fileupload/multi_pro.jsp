...
<!-- 헤더들어가는 곳 -->
		...
		<ul>
			...
			%>		
				<li>
					<!-- 다운로드할 경로 + 다운로드할 파일명(업로드한 파일명) -->
					<%-- <a href="/Fileupload/upload/<%=saveFiles.get(i)%>">--%>
					<!--파일다운로드를 처리하는 download.jsp페이지로 다운로드 요청 해야함
					다운로드할(업로드된)파일의 가상경로 전달,
					실제 다운로드할 파일명(실제 서버에 업로드된 파일명)전달 
					-->
